...
serve(async (req) => {
  ...
  try {
    ...
    const xrpl_currency_code = requestData.asset_symbol.length <= 3 
      ? requestData.asset_symbol.toUpperCase().padEnd(3, '\0')
      : Buffer.from(requestData.asset_symbol.substring(0, 20)).toString('hex').toUpperCase().padEnd(40, '0');
