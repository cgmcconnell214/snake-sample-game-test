-- CRITICAL SECURITY FIX: Address infinite recursion and KYC data protection (Fixed)
...
-- Step 6: Add security event logging for KYC access (Fixed trigger)
...
-- Add triggers for KYC access logging (Fixed - no SELECT trigger)
DROP TRIGGER IF EXISTS kyc_access_log_trigger ON public.kyc_verification;
CREATE TRIGGER kyc_access_log_trigger
    AFTER INSERT OR UPDATE OR DELETE ON public.kyc_verification
