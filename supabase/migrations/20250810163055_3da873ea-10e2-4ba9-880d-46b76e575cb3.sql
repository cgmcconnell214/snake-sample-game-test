-- Deduplicate existing enrollments before adding unique index
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Delete duplicates keeping the lowest id per (student_id, course_id)
    WITH ranked AS (
      SELECT id, student_id, course_id,
             ROW_NUMBER() OVER (PARTITION BY student_id, course_id ORDER BY id) AS rn
...
-- Re-apply the previous migration pieces safely
...
-- Course completion badge trigger
