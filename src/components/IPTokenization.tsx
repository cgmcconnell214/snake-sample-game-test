...
const IPTokenization: React.FC = () => {
  ...
  const calculateTokenMetrics = (revenue: number, valuation: number) => {
    ...
    const annualYieldPercentage = revenue > 0 ? (revenue / valuation) * 0.001 : 0.005; // 0.1% of revenue to valuation ratio, min 0.5%
  ...
  const createIPAsset = async () => {
    try {
      const metrics = calculateTokenMetrics(newAsset.annual_revenue, newAsset.valuation);
              ...
              <Card key={asset.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={asset.verification_status === 'verified' ? 'default' : 'secondary'}>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-medium capitalize">{asset.ip_type}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Annual Revenue</div>
                      <div className="font-medium">{formatCurrency(asset.annual_revenue)}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Valuation</div>
                      <div className="font-medium">{formatCurrency(asset.valuation)}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Total Tokens</div>
                      <div className="font-medium">{formatNumber(asset.total_tokens)}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium">
                        {(asset.annual_yield_percentage * 100).toFixed(2)}% Annual Yield
                      </span>
                    </div>
        ...
        <TabsContent value="portfolio" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatNumber(tokenHoldings.reduce((sum, holding) => sum + holding.tokens_held, 0))}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatNumber(tokenHoldings.reduce((sum, holding) => sum + holding.tokens_staked, 0))}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {formatCurrency(tokenHoldings.reduce((sum, holding) => sum + holding.accumulated_rewards, 0))}
                </div>
                ...
                {tokenHoldings.map((holding) => (
                  <div key={holding.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">{holding.ip_asset.description}</p>
                      <div className="flex items-center space-x-4 mt-2 text-sm">
                        <span>Owned: {formatNumber(holding.tokens_held)}</span>
                        <span>Staked: {formatNumber(holding.tokens_staked)}</span>
                        <span>Rewards: {formatCurrency(holding.accumulated_rewards)}</span>
                      </div>
                    </div>
                    <div className="flex space-x-2">
        ...
        <TabsContent value="create" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Convert your intellectual property into tradeable tokens with staking rewards
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewAsset(prev => ({ ...prev, ip_type: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewAsset(prev => ({ ...prev, annual_revenue: parseFloat(e.target.value) || 0 }))}
                  />
                </div>
                <div>
                  <label className="text-sm font-medium">Asset Valuation ($USD)</label>
                  <Input
                    type="number"
                    placeholder="10000000"
                    value={newAsset.valuation || ''}
                    onChange={(e) => setNewAsset(prev => ({ ...prev, valuation: parseFloat(e.target.value) || 0 }))}
                  />
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <div className="font-medium">
                        {formatNumber(calculateTokenMetrics(newAsset.annual_revenue, newAsset.valuation).totalTokens)}
                      </div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Tokens per $1</div>
                      <div className="font-medium">
                        {calculateTokenMetrics(newAsset.annual_revenue, newAsset.valuation).tokensPerDollar}
                      </div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Annual Yield</div>
                      <div className="font-medium">
                        {(calculateTokenMetrics(newAsset.annual_revenue, newAsset.valuation).annualYieldPercentage * 100).toFixed(2)}%
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Button 
                ...
                disabled={!newAsset.name || !newAsset.description || newAsset.valuation <= 0}
                className="w-full"
              >
