...
export function AITradingBot(): JSX.Element {
  ...
  const [currentStrategy, setCurrentStrategy] = useState<StrategyRunner | null>(
    null,
  );
    ...
    try {
      ...
      setPerformance((prev) => ({
        ...
        confidenceScore: Math.max(
          60,
          Math.min(95, prev.confidenceScore + (Math.random() - 0.5) * 5),
        ),
      }));
    } catch (error) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="p-3 bg-warning/10 border border-warning/20 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs">
                <div className="font-medium text-warning mb-1">Risk Notice</div>
                <div className="text-muted-foreground">
                  AI trading involves risk. Past performance does not guarantee
                  future results.
                </div>
              </div>
            </div>
      ...
      <Card className="bg-card/50 backdrop-blur border-border/50">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-center">
            <div
              className={`text-2xl font-bold ${performance.totalPnL >= 0 ? "text-buy" : "text-sell"}`}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              AI Confidence: {performance.confidenceScore}%
            </span>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {signals.length === 0 ? (
              <div className="text-center text-muted-foreground py-4">
                ...
                <p className="text-xs">
                  Start the bot to see AI recommendations
                </p>
              </div>
            ) : (
