...
export default function CourseBuilder() {
  ...
  const [editableCourse, setEditableCourse] = useState<Partial<Course> | null>(
    null,
  );
  ...
  const [analytics, setAnalytics] = useState<{
    ...
    free: number;
    revenue: number;
  }>({ total: 0, paid: 0, free: 0, revenue: 0 });
  ...
  const fetchCourseEnrollments = async (courseId: string) => {
    ...
    const paid = (data || []).filter(
      (e: any) => e.payment_status === "paid" && (e.payment_amount || 0) > 0,
    ).length;
    const free = (data || []).filter(
      (e: any) =>
        (e.payment_amount || 0) === 0 || e.payment_provider === "free",
    ).length;
    const revenue = (data || []).reduce(
      (sum: number, e: any) =>
        sum + (e.payment_status === "paid" ? e.payment_amount || 0 : 0),
      0,
    );
  ...
  const handleSaveSettings = async () => {
    ...
    const update = {
      ...
      price_per_student:
        editableCourse.price_per_student ?? selectedCourse.price_per_student,
      is_published:
        (editableCourse as any).is_published ?? selectedCourse.is_published,
    };
    const { data, error } = await supabase
      .from("educational_courses")
      .update(update)
    ...
    setCourses((prev) =>
      prev.map((c) => (c.id === data.id ? (data as Course) : c)),
    );
  ...
  const handleUpdateLesson = async () => {
    ...
    setLessons((prev) =>
      prev.map((l) => (l.id === data.id ? (data as Lesson) : l)),
    );
  ...
  const handleUpdateAssignment = async () => {
    ...
    setAssignments((prev) =>
      prev.map((a) => (a.id === data.id ? (data as Assignment) : a)),
    );
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            variant="outline"
                            onClick={() => {
                              setEditableCourse(selectedCourse);
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="text-center p-4 border rounded">
                          ...
                          <p className="text-2xl font-bold">
                            ${selectedCourse.price_per_student}
                          </p>
                        ...
                        <div className="space-y-2">
                          {enrollments.map((e: any) => (
                            ...
                            >
                              <div>
                                <p className="font-medium">
                                  {studentProfiles[e.student_id]
                                    ?.display_name ||
                                    studentProfiles[e.student_id]?.username ||
                                    e.student_id}
                                </p>
                                <p className="text-sm text-muted-foreground">
                                  {e.payment_provider} • {e.payment_status} • $
                                  {e.payment_amount || 0}
                                </p>
                              </div>
                ...
                <DialogContent className="max-w-xl">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <Select
                        value={
                          editableCourse?.category ?? selectedCourse.category
                        }
                        onValueChange={(v) =>
                          setEditableCourse({
                            ...(editableCourse || {}),
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        step="0.01"
                        value={
                          editableCourse?.price_per_student ??
                          selectedCourse.price_per_student
                        }
                        onChange={(e) =>
                          setEditableCourse({
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={
                          (editableCourse?.is_published as any) ??
                          selectedCourse.is_published
                        }
                        onChange={(e) =>
                          setEditableCourse({
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={editingAssignment.attempts_allowed}
                            onChange={(e) =>
                              setEditingAssignment({
                                ...editingAssignment,
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={editingAssignment.passing_score}
                            onChange={(e) =>
                              setEditingAssignment({
                                ...editingAssignment,
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={editingAssignment.time_limit_minutes || 0}
                            onChange={(e) =>
                              setEditingAssignment({
                                ...editingAssignment,
                                time_limit_minutes:
