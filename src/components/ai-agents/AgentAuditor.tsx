...
export function AgentAuditor() {
  ...
  const runAudit = async () => {
    ...
    try {
      ...
      const workflowTest = await testWorkflowPersistence(
        createTest.details?.agentId,
      );
      results.push(workflowTest);

      // Test 4: Agent Execution
      setCurrentTest("Testing agent execution...");
      const executionTest = await testAgentExecution(
        createTest.details?.agentId,
      );
      ...
      const apiKeyTest = await testAPIKeyGeneration(
        createTest.details?.agentId,
      );
  ...
  const testWorkflowPersistence = async (
    ...
    try {
      ...
      const isWorkflowPersisted =
        workflowData && workflowData.steps && workflowData.steps.length === 2;
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${getStatusColor(auditResults.overall)}`}
                  />
                  Audit Summary
                </CardTitle>
              </CardHeader>
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-500">
                        {
                          auditResults.tests.filter((t) => t.status === "pass")
                            .length
                        }
                      </div>
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-red-500">
                        {
                          auditResults.tests.filter((t) => t.status === "fail")
                            .length
                        }
                      </div>
                      <div className="text-sm text-muted-foreground">
