...
import { CheckCircle, XCircle, AlertCircle, Bot, DollarSign } from "lucide-react";
    ...
    try {
      ...
      if (createError) {
        ...
        setTestResults(prev => [...prev, {
          ...
          data: { foundAgents: userAgents?.length, targetAgent: createdAgent.id }
        }]);
        ...
        // Cleanup test data
        await supabase.from('ai_agents').update({ is_active: false }).eq('id', createdAgent.id);
      }
